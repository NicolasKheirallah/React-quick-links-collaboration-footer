  ...
  const handleImportLinks = useCallback(() => {
    ...
    fileInput.onchange = async (event: any) => {
      ...
      try {
        ...
        await onLinksImported(validLinks);
        onStatusUpdate(
          `Import completed: ${validCount} links processed${invalidCount > 0 ? `, ${invalidCount} invalid links skipped` : ''}`
        );
  ...
  const handleExportAllLinks = useCallback(() => {
    try {
      ...
      if (allLinks.length === 0) {
        onStatusUpdate('No links available to export', true);
        return;
      }
